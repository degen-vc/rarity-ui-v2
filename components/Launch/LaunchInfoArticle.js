...
const LaunchInfoArticle = ({skinBalance = '0', currentAdventurer = ''}) => (
	<BoxWithTitle title={`DRESS YOUR ${USER} FOR THE PARTY`} className={'mt-12'}>
		<article className={'text-sm opacity-80 mt-10 sm:mt-0'}>
			<p className={'pb-4'}>{`The costumes are generated at random as one of the 11 ${USER} classes.`}</p>
			<p className={'pb-4'}>{'As an example, if you own a Paladin and mint one costume, you may not receive a Paladin costume.'}</p>
			<p className={'pb-4'}>{`If you have the matching ${USER}, you can immediately connect the costume.`}</p>
			...
			<p className={'pb-4'}>{`You could mint an ${USER} to go along with that costume.`}</p>
