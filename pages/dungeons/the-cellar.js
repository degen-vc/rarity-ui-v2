...
function	DialogChoices({router, step, stepAuto, ratEscaped, adventurerWon, expectedLoot, loot}) {
	if (ratEscaped) {
		return (
			<DialogBox
				options={[
					{label: 'THE BIG UGLY RAT HAS ESCAPED', onClick: () => router.push('/world/quest?tab=the-cellar')},
				]} />
		);
	}
	if (adventurerWon) {
		if (expectedLoot === 0) {
			return (
				<DialogBox
					options={[
						{label: 'YOU HAVE DEFEATED THE RAT ! UNFORTUNATELY, THERE IS NOTHING TO RECOVER', onClick: () => router.push('/world/quest?tab=the-cellar')},
					]} />
			);
		}
		return (
	...
	return (
		<DialogBox
			options={[
				...
				{label: 'ESCAPE', onClick: () => router.push('/world/quest?tab=the-cellar')},
			]} />
	);
}
...
function	Index({dungeon, adventurer, router}) {
	...
	async function	stepAuto() {
		...
		for (let index = STEP_LIMIT; index >= fightStep; index--) {
			...
			if (_dungeonHealth <= 0) {
				set_logs(l => [...l, 'Vigo will be happy, you killed this Ugly rat!']);
	...
	return (
		<section className={'relative'}>
			<div className={`absolute bg-black inset-0 z-10 -top-24 -left-4 -right-4 flex flex-col items-center min-h-screen transition-opacity duration-1000 ${adventurerHealth <= 0 ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
				...
				<p className={'text-base text-white pt-8 mx-4 md:mx-0 max-w-screen-sm text-center'}>{'After some time, the rat returns to his hole and Vigo the tavernkeeper, worried, finds you lying on the floor'}</p>
				<Link href={'/world/quest?tab=the-cellar'}>
					...
					loot={() => {
						lootDungeonTheCellar({
							...
							if (router.pathname === '/dungeons/the-cellar')						
								router.push('/world/quest?tab=the-cellar');
						});
					}}
				/>
			</div>
